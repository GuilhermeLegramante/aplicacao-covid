...
if ($mysql->connect_error) {
  ...
  if (is_uploaded_file($_FILES["fotoDoc"]["tmp_name"])) {
    ...
    $buscaCpf = "SELECT * FROM `voluntarios` where cpf = '{$cpf}'";

    $reg = $mysql->query($buscaCpf);

    if ($reg->num_rows > 0) {
      $response["MESSAGE"] = "Erro. CPF já cadastrado.";
      $response["STATUS"] = 500;
    } else {
      $sql = "INSERT INTO voluntarios (nome,cpf,email,senha,telefone,cep,rua,numero,bairro,cidade,uf,raioAtuacao,identidade,fotoDoc,latitude,longitude) 
